...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class ShelfDefBLL
    {
        ...
        CarrierDao cassetteDataDao = null;
        ...
        public void start(SCApplication scApp)
        {
            ...
            cassetteDataDao = scApp.CarrierDao;
        }
        ...
        public bool updateStatus(string shelf_id, string status)
        {
            try
            {
                ...
                {
                    scApp.LineService.ProcessAlarmReport("",scApp.getEQObjCacheManager().getLine().LINE_ID, ((int)AlarmLst.LINE_NotEmptyShelf).ToString(), sc.ProtocolFormat.OHTMessage.ErrorStatus.ErrReset
                        ,"No empty shelf left.");
                }
            }
            catch (Exception ex)
        ...
        public UInt16 getCurrentShelfCapacity()
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                ...
                cassette.ForEach(x =>
                {
                    i = i + shelfdefDao.LoadShelfDef(con).Where(y => y.Enable == "Y" && y.ShelfID == x.LOCATION).Count();
                });
