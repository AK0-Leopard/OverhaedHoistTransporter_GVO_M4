...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    ...
    public class PortStationDao : DaoBase
    {
        ...
        public List<APORTSTATION> loadAllNonEQPort(DBConnection_EF con)
        {
            var query = from port in con.APORTSTATION
                        where port.PORT_TYPE !=  1
                        orderby port.PORT_ID
                        select port;
            return query.ToList();
        }
        ...
        public int getAPortstationStageCountByID(string id)
        {
            try
            {
                DataTable dt = SCApplication.getInstance().OHxCConfig.Tables["APORTSTATIONSETTING"];
                var query = from c in dt.AsEnumerable()
                            where c.Field<string>("PORTSTATION_ID").Trim() == id.Trim()
                            select c.Field<string>("STAGE_COUNT");
                List<string> strList = query.ToList();
                return int.Parse(strList.FirstOrDefault());
            }
            catch (Exception ex)
            {
                logger.Warn(ex);
