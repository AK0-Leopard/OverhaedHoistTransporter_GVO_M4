    ...
    public class PLCAliveTimer : ITimerAction
    {
        ...
        public override void doProcess(object obj)
        {
            if (System.Threading.Interlocked.Exchange(ref syncPoint, 1) == 0)
            {
                try
                {
                    APORT port = scApp.getEQObjCacheManager().getPortByPortID("MASTER_PLC");
                    systemInfoMapAction = port
                    ...
                    {
                        if(port.heartbeat == false)
                        {
                            systemInfoMapAction.PLC_SetHeartbeat(true);
                        }
