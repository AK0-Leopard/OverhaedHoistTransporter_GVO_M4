    ...
    class PLCSystemInfoMapAction : IValueDefMapAction
    {
        ...
        private void PLCSystem_onHeartBeat(object sender, ValueChangedEventArgs e)
        {
            ...
            try
            {
                ...
                //    NLog.LogManager.GetCurrentClassLogger().Info(function.ToString());
                //}
                masterPLC.heartbeat = function.PLCHeartBeat;
                NLog.LogManager.GetCurrentClassLogger().Info(function.ToString());
            }
            catch (Exception ex)
            {
        ...
        public void Port_Timeout_Set(int t1_timeout, int t2_timeout, int t3_timeout, int t4_timeout, int t5_timeout, int t6_timeout)
        {
            var function = scApp.getFunBaseObj<PortPLCControl_TimeoutSetting>(masterPLC.PORT_ID) as PortPLCControl_TimeoutSetting;
            try
            {
                //1.建立各個Function物件
                function.T1_Timeout = t1_timeout;
                function.T2_Timeout = t2_timeout;
                function.T3_Timeout = t3_timeout;
                function.T4_Timeout = t4_timeout;
                function.T5_Timeout = t5_timeout;
                function.T6_Timeout = t6_timeout;

                function.Write(bcfApp, SCAppConstants.EQPT_OBJECT_CATE_PORT, masterPLC.PORT_ID);
                function.Timestamp = DateTime.Now;
            ...
            {
                scApp.putFunBaseObj<PortPLCControl_TimeoutSetting>(function);
            }
        }
