...
{//test
    ...
    public partial class BCMainForm : Form
    {
        ...
        {
            //設定 Form Title 顯示的字
            this.Text = this.Text + " - " + bcApp.SCApplication.BC_ID;
        ...
        private void portDataToolStripMenuItem_Click(object sender, EventArgs e)
        {
            openForm(typeof(TestGetPortData).Name, true, false);
        }

        private void maintainDeviceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            openForm(typeof(MaintainDeviceForm).Name, true, false);
        }

        private void uASToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (!BCUtility.doLogin(this, bcApp, BCAppConstants.FUNC_USER_MANAGEMENT))
            {
                return;
